...
import { User, Calendar, School, BookOpen, Star, Clock, FileText, Filter, ChevronDown, ChevronUp } from 'lucide-react';
...
const StudentProfile: React.FC<StudentProfileProps> = ({ studentId }) => {
  ...
  const memProgress = student.overAllMem > 0 ? (completedMem / student.overAllMem) * 100 : 0;
        ...
        <CardContent className="p-6">
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-lg text-muted-foreground mb-4">{student.name_eng}</p>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-primary" />
                  <span className="text-sm">
                    تاريخ الميلاد: {new Date(student.bDate).toLocaleDateString('ar-SA')}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">عدد السجلات: {logsArray.length}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <Card className="shadow-card">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-2xl font-bold text-primary">{student.overAllMem}</p>
                <p className="text-sm text-muted-foreground font-arabic">إجمالي الحفظ</p>
        ...
        <Card className="shadow-card">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-2xl font-bold text-accent">{student.newMem}</p>
                <p className="text-sm text-muted-foreground font-arabic">الحفظ الجديد</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-card">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-2xl font-bold text-primary">{completedRev}</p>
                <p className="text-sm text-muted-foreground font-arabic">المراجعة المكتملة</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="shadow-card">
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <CardTitle className="font-arabic text-primary">سجلات الطالب</CardTitle>
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="flex items-center gap-2">
                <Filter className="h-4 w-4 text-muted-foreground" />
                <Label htmlFor="date-filter" className="text-sm">تصفية بالتاريخ:</Label>
                <Input
                  id="date-filter"
                  type="date"
                  value={selectedDate}
        ...
        <CardContent>
          {isLoadingLogs ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="mr-2 text-muted-foreground">جارٍ تحميل السجلات...</span>
            </div>
          ) : displayedLogs.length === 0 ? (
            <div className="text-center py-8">
              <FileText className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
              <p className="text-muted-foreground font-arabic">
                {selectedDate ? 'لا توجد سجلات لهذا التاريخ' : 'لا توجد سجلات متاحة'}
              </p>
            </div>
          ) : (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm text-muted-foreground">
                            {new Date(log.time).toLocaleDateString('ar-SA')} - {new Date(log.time).toLocaleTimeString('ar-SA')}
                          </span>
                        </div>
                        
                        <div className="flex flex-wrap gap-2 mb-2">
                          <Badge variant={log.memDone === 'true' ? 'default' : 'secondary'}>
                            حفظ: {log.memDone === 'true' ? 'مكتمل' : 'غير مكتمل'}
                          </Badge>
                          {log.memGrade && (
                          ...
                          <Badge variant={log.revDone === 'true' ? 'default' : 'secondary'}>
                            مراجعة: {log.revDone === 'true' ? 'مكتمل' : 'غير مكتمل'}
                          </Badge>
